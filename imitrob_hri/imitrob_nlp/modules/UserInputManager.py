...
@author: Zdenek Kasner, Karla Stepanova, Jan Behrens
@amail: karla.stepanova@cvut.cz
"""
import json
import os
...
class UserInputManager(CrowModule):
    ...
    def buffered_say_old(self, text='',  say=True, screen=True, flush=False) -> None:
        if not flush:
            def say():
                self.say(text, say, screen)
            self.__class__.buffer.append(say)
        else:
            for bla in self.__class__.buffer:
                bla()
            self.__class__.buffer = []


    def buffered_say(self, text='',  say=3, screen=1, flush=False, level=3) -> None:
        if not flush:
            # print('no flush newsay')
            def newsay(newlevel):
                self.say(text, say=say<=newlevel, screen=screen<=newlevel)
            self.__class__.buffer.append(newsay)
        else:
            for bla in self.__class__.buffer:
                bla(newlevel=level)
            self.__class__.buffer = []
    ...
    def load_file(self,file, package = 'imitrob_hri',folder = 'imitrob_nlp/utils'):
